...
下面是从 [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines) 整理的通用规范。它**不是 Vase 的规范**——它是语言层面的参考，遇到 Vase 自己的规矩时**一律以 Vase 的为准**。

## 与本仓库直接冲突的地方（不要照做）

| 这份文件说 | Vase 实际怎么做 |
|---|---|
| **E.* 整节：用异常报错**（E.2 抛、E.14 自定义异常类型、E.15 按值抛按引用接、E.17 不要到处 catch） | **全项目关闭异常**（`/EHs-c-` / `-fno-exceptions`）。失败一律 `Result<T>` / `Error` 显式返回；编程错误走 `detail::ProgrammerError()`。见 `error-handling.md` |
| **`noexcept`（F.6 / E.12）** | 无异常项目里它的表达力有限——默认就是不抛的。写它之前先想清楚要传达什么 |
| **NL.10 偏好 `underscore_style`** | Vase 是 **CamelCase**：类 / 函数 / 成员 `CamelCase`，参数 / 局部 `camelBack`，全局 `g` + `CamelCase`，常量与 constexpr `k` + `CamelCase`。**由 `.clang-tidy` 的 `readability-identifier-naming.*` 强制**，以根 `CLAUDE.md` 的命名表为准 |
| 成员变量用尾下划线（`name_`） | **成员名不带任何前后缀**。`Message_` / `mCount` / `_private` 全部违规。成员与访问器撞名时**让成员改名**（`Message()` 撞车 → 成员叫 `Text`） |
| **NL.5 避免在名字里编码类型（无匈牙利）** | 一致。但 `k` / `g` 前缀**不是匈牙利**——它们是 Vase 命名规范的一部分 |
| **SF.8 用 include guard** | 仓库用 **`#pragma once`** |
| **R.11 避免显式 `new` / `delete`；R.5 尽量别堆分配** | 一致（裸 `new` / `delete` 表达式过不了门禁），但 Vase 还有第三条路：**Effect 是 placement-new 到 `ScopePool` 里的**，销毁路径 = `Recycle()` + 显式析构调用，**不走 `delete`**。见 `ownership-lifetime.md` |
| **C.12 别把成员设成 `const` 或引用** | 那条只在"想保留拷贝 / 移动"时适用。`Pod` 有引用成员（`Pool` / `Counters`）且**刻意删掉值语义**——它由 Host 以 `unique_ptr` 独占 |
| **Per.* 性能条目** | 以 `performance.md` 的口径为准：**不禁止抽象，要求每个抽象能解释自己的成本**。"别过早优化"在这里的意思是"没有测量就不要声称性能" |

## 与 Vase 一致、值得强化的部分

RAII、默认不可变（`const` / `constexpr`）、类型安全、表达意图、降低复杂度、值语义优先——这六条正是 Vase 的底色。另外这几条在本仓库被反复印证：

- **C.20 规则零 / C.21 规则五**：`Plugin`、`Context`、`Pod`、`EffectScope`、`IEffect` 全都显式处置了四个特殊成员（多半是全删），从不留隐式默认。
- **C.35 基类析构**：`Plugin` 是 public virtual；`IEffect` 是 **protected 非虚**——`delete IEffect*` 编不过，销毁权只属于 `EffectScope`。这是个好例子：**C.35 的两种形态都有用武之地，选哪种取决于"谁有权销毁"。**
- **T.10 / T.11 用概念约束模板**：`HasServiceIdentity` / `HasEventIdentity` 是仓库里的实例，且它们约束的是**静态数据成员的存在性与类型**，不是常见的接口形状。
- **C.46 单参构造 `explicit`**、**ES.23 优先 `{}` 初始化**、**ES.20 总是初始化**、**Enum.3 用 `enum class`**：仓库里全面执行。
- **SL.io.50 用 `'\n'` 而非 `std::endl`**。

## 冲突时怎么办

**Vase 仓库里的既有写法 + 根 `CLAUDE.md` + 姊妹 reference 文件，构成优先级链；这份文件排在最后。** 若你发现这里某条通用建议在 Vase 里看起来"更对"，先提出讨论，不要直接改既有代码。
